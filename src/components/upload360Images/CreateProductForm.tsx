...
export default function CreateProduct() {
  ...
  const handleArrayInput = (e: ChangeEvent<HTMLInputElement>, field: keyof ProductFormData) => {
    const values = e.target.value.split(",").map((v) => v.trim());
    setForm((prev) => ({ ...prev, [field]: values }));
  };

  const handleFileChange = (e: ChangeEvent<HTMLInputElement>, type: "regular" | "360") => {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl mx-auto p-6 bg-white rounded shadow space-y-6">
      ...
      <div>
        ...
        <input type="file" multiple onChange={(e) => handleFileChange(e, "regular")} />
      ...
      <div>
        ...
        <input type="file" multiple onChange={(e) => handleFileChange(e, "360")} />
        <div className="flex flex-wrap mt-2 gap-2">
